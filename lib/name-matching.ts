import { normalizeKey, normalizeWhitespace } from "@/lib/utils";
...
export function getNameMatchKey(value: string | null | undefined) {
  if (!value) {
    return "";
  }

  return normalizeKey(getFirstLastIdentity(value));
}
...
export function splitEmployeeName(value: string) {
  ...
  if (trimmed.includes(",")) {
    const [lastName, firstName] = trimmed.split(",").map((part) => normalizeWhitespace(part));
    return { firstName, lastName };
  }
