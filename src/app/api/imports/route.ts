...
async function importOffboarding(
  ...
  const managerPayload: Database["public"]["Tables"]["managers"]["Insert"][] = Array.from(
    new Map(
  ...
  const employeePayload: Database["public"]["Tables"]["employees"]["Insert"][] = Array.from(
    normalizedRows.reduce((map, row) => {
      ...
      return map;
    }, new Map<string, Database["public"]["Tables"]["employees"]["Insert"]>()).values(),
  );
  ...
  const employeeMap = new Map<string, NonNullable<typeof employees>[number]>();
